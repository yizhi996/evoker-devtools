<template>
  <div
    class="w-full"
    :style="{ height, 'background-color': tabBarStyle.backgroundColor }"
  >
    <div
      class="w-full h-[1px]"
      :style="{ 'background-color': tabBarTopLineColor }"
    ></div>
    <div
    ...
    >
      ...
      >
        <img
          class="w-[25px] h-[25px]"
          :src="iconPath(i, items.iconPath, items.selectedIconPath)"
        />
        <span
          class="text-xs"
...
const iconPath = (index: number, iconPath?: string, selectedIconPath?: string) => {
  ...
  const filePath = path.join(getUserDataPath(), `App/${window.project.appId}/dist/${src}`)
