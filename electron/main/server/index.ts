...
export class ProjectDevServer {
  ...
  private connection() {
    this.ws = new ws.WebSocket(`ws://127.0.0.1:${this.port}`)
  ...
  private checkVersion(body: Buffer | ArrayBuffer | Buffer[]) {
    ...
    this.ws.readyState == ws.OPEN && this.ws.send(JSON.stringify(data))
  }
